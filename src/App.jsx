...
/**
 * Utility to convert numbers to standard Arabic numeral digits for verse endpoints
 */
...
const App = () => {
  ...
  const [isBookView, setIsBookView] = useState(false); // New: Track book pagination mode
  ...
  useEffect(() => {
    if (ayahs[activeAyahIndex]) {
      ...
      // Auto-scroll active verse into view if we are inside the book layout mode
      if (isBookView && mode === 'listen') {
        const activeElement = document.getElementById(`book-ayah-${activeAyahIndex}`);
        if (activeElement) {
      ...
      <div className="min-h-screen w-full flex flex-col items-center justify-center bg-[#fdfaf3] p-6 pattern-bg">
        <div className="w-full max-w-lg space-y-12">
          ...
          <div className="space-y-4">
            <button onClick={() => { setMode('manual'); setIsBookView(false); setView('browser'); }} className="w-full group p-8 bg-[#1e3a31] rounded-[2rem] text-white flex items-center justify-between transition-all hover:translate-y-[-4px] shadow-xl shadow-emerald-900/30">
            ...
            <button onClick={() => { setMode('listen'); setIsBookView(false); setView('browser'); }} className="w-full group p-8 bg-[#c29b40] rounded-[2rem] text-[#1e3a31] flex items-center justify-between transition-all hover:translate-y-[-4px] shadow-xl shadow-yellow-900/20">
              <div className="text-left">
        ...
        {!selectedSurah ? (
          ...
          <>
            <div className="flex items-center gap-4">
              <button onClick={() => { setSelectedSurah(null); setIsPlaying(false); audioRef.current.pause(); setIsBookView(false); }} className="flex items-center gap-2 text-[#8b7d6b] hover:text-[#1e3a31] transition-colors">
                <ChevronLeft size={24} /> <span className="font-heading text-lg">Return to Library</span>
              </button>

              {/* Mode-dependent Book UI toggle */}
              {mode === 'listen' && (
                <button 
                  onClick={() => setIsBookView(!isBookView)}
                  className={`ml-4 px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 border transition-all ${
                    isBookView 
                    ? 'bg-[#1e3a31] border-[#c29b40] text-[#c29b40]' 
                    : 'bg-white border-[#e8dfca] text-[#8b7d6b] hover:border-[#c29b40]'
                  }`}
                >
                  <Book size={16} />
                  <span>{isBookView ? "Flashcard View" : "Book View"}</span>
                </button>
              )}
            </div>
      ...
      <main className="w-full max-w-6xl flex-1 flex flex-col items-center justify-center p-6 text-center">
        {!selectedSurah ? (
          ...
          <div className="w-full space-y-12 py-6">
            ...
            {/* Conditional Rendering: Book View layout vs Flashcard View Layout */}
            {isBookView && mode === 'listen' ? (
              <div className="w-full max-w-4xl bg-white border border-[#e8dfca] rounded-[3rem] p-10 md:p-16 shadow-xl mx-auto text-right">
                {/* Bismillah Header inside Book View */}
                {selectedSurah.number !== 1 && selectedSurah.number !== 9 && (
                  <div className="text-center text-[#c29b40] font-arabic text-3xl mb-12">
                    {BISMILLAH_ARABIC}
                  </div>
                )}
                
                {/* Flowing Page Layout Text block */}
                <div className="font-arabic text-3xl md:text-4xl leading-[2.6] text-[#1e3a31] text-justify tracking-wide px-4" style={{ direction: 'rtl' }}>
                  {ayahs.map((ayah, idx) => {
                    const isCurrent = idx === activeAyahIndex;
                    return (
                      <span 
                        key={idx}
                        id={`book-ayah-${idx}`}
                        onClick={() => setActiveAyahIndex(idx)}
                        className={`cursor-pointer transition-all duration-300 mx-1 rounded px-1 ${
                          isCurrent 
                            ? 'bg-[#c29b40]/20 text-[#c29b40] font-bold shadow-sm ring-2 ring-[#c29b40]/30 scale-105 inline-block' 
                            : 'hover:bg-slate-100'
                        }`}
                      >
                        {ayah.text}
                        <span className="mx-2 text-[#c29b40] inline-block font-sans text-xl opacity-80 select-none">
                ...
                {/* Live Translation Footer inside Book view matching current active audio */}
                <div className="mt-12 pt-8 border-t border-[#e8dfca] text-center">
                  <p className="text-[#8b7d6b] text-xs uppercase font-black tracking-widest mb-2">Translation (Verse {activeAyahIndex + 1})</p>
                  <p className="text-[#5c5346] text-lg font-body italic max-w-2xl mx-auto leading-relaxed">
                    "{ayahs[activeAyahIndex]?.translation}"
                  </p>
                </div>
              </div>
            ) : (
              /* Original Flashcard Layout Template */
