...
const App = () => {
  ...
  const [verseSearch, setVerseSearch] = useState(""); // Verse search state
  ...
  // Presentation layouts for continuous listening mode
  const [listenLayout, setListenLayout] = useState('single'); // 'single' or 'book'
  ...
  // Sync Book Page View window when active audio verse updates on listen mode
  useEffect(() => {
    if (ayahs[activeAyahIndex]) {
      ...
      if (mode === 'listen' && listenLayout === 'book') {
        const structuralPage = Math.floor(activeAyahIndex / versesPerPage);
        setBookPage(structuralPage);
      }

      if (isPlaying) audio.play().catch(() => {});
    }
  }, [activeAyahIndex, ayahs, listenLayout, mode]);
  ...
  // Compute values for book view page pagination segment 
  const paginatedBookAyahs = useMemo(() => {
    const startIndex = bookPage * versesPerPage;
    return ayahs.slice(startIndex, startIndex + versesPerPage);
  }, [ayahs, bookPage]);

  const totalBookPages = Math.ceil(ayahs.length / versesPerPage);
      ...
      <div className="min-h-screen w-full flex flex-col items-center justify-center bg-[#fdfaf3] p-6 pattern-bg">
        <div className="w-full max-w-lg space-y-12">
          ...
          <div className="space-y-4">
            ...
            <button onClick={() => { setMode('listen'); setView('browser'); setListenLayout('single'); }} className="w-full group p-8 bg-[#c29b40] rounded-[2rem] text-[#1e3a31] flex items-center justify-between transition-all hover:translate-y-[-4px] shadow-xl shadow-yellow-900/20">
              <div className="text-left">
      ...
      <main className="w-full max-w-6xl flex-1 flex flex-col items-center justify-center p-6 text-center">
        {!selectedSurah ? (
          ...
          <div className="w-full space-y-12 py-6">
            ...
            {/* Conditional Book View Rendering for Listen Mode */}
            {mode === 'listen' && listenLayout === 'book' ? (
              <div className="w-full max-w-5xl mx-auto bg-white rounded-[3rem] border border-[#e8dfca] shadow-2xl p-8 md:p-12 space-y-8">
                <div className="flex justify-between items-center border-b border-[#e8dfca] pb-4">
                  <span className="text-xs font-black text-[#c29b40] uppercase tracking-widest flex items-center gap-2">
                    <Book size={14}/> Page {bookPage + 1} of {totalBookPages}
                  </span>
                  <div className="flex gap-2">
                    <button 
                      onClick={() => setBookPage(p => Math.max(0, p - 1))} 
                      disabled={bookPage === 0}
                      className="p-2 border border-[#e8dfca] rounded-xl hover:bg-[#faf7f0] disabled:opacity-30"
                    >
                      <ChevronLeft size={16}/>
                    </button>
                    <button 
                      onClick={() => setBookPage(p => Math.min(totalBookPages - 1, p + 1))} 
                ...
                {/* Main page canvas containing flowing block verses layout */}
                <div className="text-right space-y-8 py-4" style={{ direction: 'rtl' }}>
                  {bookPage === 0 && selectedSurah.number !== 1 && selectedSurah.number !== 9 && (
                    <div className="text-center text-[#c29b40]/60 font-arabic text-2xl my-4">
                      {BISMILLAH_ARABIC}
                    </div>
                  )}
                  <p className="font-arabic text-xl md:text-3xl leading-[2.5] text-[#1e3a31] text-justify">
                    {paginatedBookAyahs.map((ayah) => {
                      const absoluteIndex = (bookPage * versesPerPage) + paginatedBookAyahs.indexOf(ayah);
                      const isVerseActive = absoluteIndex === activeAyahIndex;
                      
                      return (
                        <span 
                          key={ayah.number}
                          onClick={() => setActiveAyahIndex(absoluteIndex)}
                          className={`cursor-pointer px-1 rounded-lg transition-all mx-1 ${
                            isVerseActive 
                            ? 'bg-[#c29b40]/20 text-[#1e3a31] ring-2 ring-[#c29b40]/40 font-bold scale-105 inline-block' 
                            : 'hover:bg-slate-100'
                          }`}
                        >
                          {ayah.text}
                          <span className="inline-flex items-center justify-center w-8 h-8 rounded-full border border-[#c29b40] text-xs font-sans text-[#c29b40] mx-2 font-bold select-none align-middle">
                            {ayah.number}
                          </span>
                        </span>
                      );
              ...
              <div className="flex items-center gap-10">
                ...
                {mode === 'listen' ? (
                  <button 
                    ...
                    className={`w-16 h-16 rounded-full flex items-center justify-center transition-all ${listenLayout === 'book' ? 'bg-[#c29b40] text-[#1e3a31] shadow-lg scale-110' : 'bg-white border border-[#e8dfca] text-[#1e3a31] hover:bg-[#faf7f0]'}`}
                    title={listenLayout === 'book' ? "Switch to Focus Mode" : "Switch to Book View"}
                  >
                    <Book size={32} />
                  </button>
                ) : (
